...
**Read the change as a narrative.** The guided tab groups the diff into ordered
...
Claude Code running headless on your machine. Clicking a file jumps to its diff.

**Comment on lines.** Click a line number, write a comment. Threads live beside the
code. You resolve them; nothing else can.
...
guide leaves the rail empty and a Retry button, with the diff fully usable.
